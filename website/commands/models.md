...
| Family      | Astra / Sol / Terra / Luna / Fable / Opus / Sonnet / Haiku / GPT-5 / GPT-4 / … (derived from the name). |
...
- **Family detection.** `model_family` is a substring match on the model tag:
  ...
  `Sol`, or `Luna`, GPT-5.6 model IDs → `Sol`, `Terra`, or `Luna`, other `gpt-5`/`gpt5` → `GPT-5`,
