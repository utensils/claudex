...
| `crates/claudex/src/types.rs`          | `TokenUsage`, `ModelPricing` (Claude, GPT-6, GPT-5.6, legacy GPT, supported Gemini Flash, and Grok 4.7 tiers). `cost_for_model` is the single source of truth; providers can supply their own `embedded_cost`.                                                                              |
