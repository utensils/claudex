...
| `crates/claudex/src/types.rs`          | `TokenUsage`, `ModelPricing` (Fable/Opus/Sonnet/Haiku + GPT-6 Astra, GPT-5.6 Sol/Terra/Luna and other gpt-5/gpt-4 tiers). `cost_for_model` is the single source of truth; providers can supply their own `embedded_cost`.                                                                   |
