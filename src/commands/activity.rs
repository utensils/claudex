...
fn print_sessions(rows: &[crate::index::IndexedSession]) {
    if rows.is_empty() {
        return;
    }
...
fn print_prs(rows: &[crate::index::PrLinkRow]) {
    if rows.is_empty() {
        return;
    }
...
fn print_files(rows: &[crate::index::FileModRow]) {
    if rows.is_empty() {
        return;
    }
...
fn print_slow(rows: &[crate::index::TurnStatsRow]) {
    if rows.is_empty() {
        return;
    }
