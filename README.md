...
Fetches a prebuilt, stripped, SHA256-verified binary into `~/.local/bin/claudex`. Override with `CLAUDEX_VERSION=v0.6.0` or `CLAUDEX_INSTALL_DIR=/usr/local/bin`. <!-- x-release-please-version -->
...
```bash
cargo install --git https://github.com/utensils/claudex --tag v0.6.0 claudex
```
...
- [Providers & filtering](https://utensils.io/claudex/guide/providers) — the three providers, retention, and the shared filter flags.
